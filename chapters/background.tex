...
While it also as scripting support, it varies greatly from Bitcoin's in that its build-in programming language is
Turing-complete, value-aware and allows keeping internal state. %TODO PICKUP!
...
% TODO idk what the right section for this is

\subsection{zkSNARKs on Blockchains}\label{subsec:onchain-snarks}

% TODO ref eth gas
Smart contracts quickly result in high costs for callers due to gas costs that increase with complexity of the
...
\textit{ZoKrates}~\cite{zokrates_repo} allows writing off-chain programs (which can be tested locally) for a verifier.
% TODO you have to ahve emntioned solidity by know or have to have ref'd it
The programs can be exported as Solidity programs~\cite{solidity_repo} which can then be deployed to the Ethereum
network.
