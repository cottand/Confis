...
\begin{itemize}
    ...
    A transaction performed by a payer will typically have three outputs: the payee's address, the payer's remaining
    balance, and a small fee for the miner.
...
\section{Smart Contracts in Ethereum}\label{sec:ethereum}
...
The scripting language is a stack-based low-level bytecode language called \textit{EVM language}.
The name of the coin of the Ethereum Ecosystem is \textit{ether}.

Ethereum's state is encoded in \textit{accounts} - an object with its own address that contains the account's
...
Therefore, when executing a smart contract its cost will be determined by the computation it requires (more computation
means more gas!).
...
verifier.
% TODO you have to have emntioned solidity by know or have to have ref'd it
