...
It does not require using libraries in order to perform operations -- mostly because there are no such operations, but partly thanks to operator overloading like in \texttt{`today + 3.months'}.
...
It is able to perform complex floating-point arithmetic in order to compute interest rates and fees.\\
...
Overall, Confis is not that lacking when compared to Symboleo as it does allow specifying numerical constants, except without having to specify their types.
