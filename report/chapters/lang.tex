...
The idea of rendering a more traditional human-readable version of the contract generated from the Confis code has two main advantages: the generated text can be used in place of a traditional contract (so a recipient or reviewer of the document does not need to use Confis);
...
\subsection{Implementation}\label{subsec:editor-implementation}
...
Take the example of~\autoref{fig:haskellBasicDsl} or~\autoref{fig:kotlinHtmlDsl}.
These files are not limited to their DSLs -- they also contain the entrypoints of their programs in their host languages (\texttt{`main ='} in Haskell and \texttt{`fun main()'} in Kotlin).
On the other hand~\autoref{fig:gradleGroovyDSL} has no traces of the Groovy language: we only see the Gradle DSL.
